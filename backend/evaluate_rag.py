...
# Import hệ thống RAG
from src.rag import llm_client, vector_store
...
def main():
    ...
    def evaluate_rag_task(item):
        ...
        result = {"input": question, "output": answer, "context": contexts}
    ...
    try:
        ...
        print(
            "Lỗi: Không thể đọc file JSON (JSON decoding failed). File có thể bị rỗng hoặc sai định dạng."
        )
        dataset_records = []
    ...
    print("\n--- ĐÁNH GIÁ HOÀN TẤT ---")
    print(
        "1. Dashboard: Kết quả dạng bảng (so sánh & điểm số) đã tự động đẩy lên Comet Opik Dashboard."
    )
    print(
        f"2. Local File: Dữ liệu câu trả lời (Câu hỏi, RAG trả lời, Đáp án chuẩn...) đã được lưu tại: {output_file}"
    )
