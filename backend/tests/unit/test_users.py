...
@pytest.fixture
def mock_user_db():
    with patch("src.routers.auth.login.users_collection") as mock_users, \
         patch("src.routers.auth.login.classes_collection") as mock_classes:
...
def test_list_teachers_success(mock_user_session, mock_user_db):
    ...
    mock_users.find.return_value.__aiter__.return_value = [
        {"_id": "teacher1", "name": "Teacher One", "email": "t1@example.com", "subjects": ["Math"]}
    ]
    ...
    mock_st_cursor.__aiter__.return_value = [
        {"_id": "class2", "name": "11B1", "subject_teachers": [{"teacher_id": "teacher1", "subject": "Math"}]}
    ]
