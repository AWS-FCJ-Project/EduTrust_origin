...
const ExamPageClient = () => {
    ...
    const [examStep, setExamStep] = useState<'camera' | 'key' | 'exam'>('camera');
    ...
    }, [timeLeft, isSubmitted, examStep, submitExam]);
        ...
        <div className="fixed inset-0 z-[10000] flex flex-col w-full h-screen bg-gray-50 font-sans overflow-hidden text-sm">
                                ...
                                <div key={qIdx} className="bg-white rounded-[2rem] shadow-sm border border-gray-200/60 overflow-hidden hover:shadow-md transition-shadow duration-300">
                                    <div className="p-7 border-b border-gray-50 bg-white">
                                        <div className="flex items-start gap-4">
                                            ...
                                            <h3 className="text-lg font-bold text-gray-800 leading-snug pt-0.5">{q.q}</h3>
                                        </div>
                                    </div>
                            ...
                            {Array.from({ length: totalQuestions }).map((_, i) => (
                                <button 
                                    ...
                                    className={`h-11 rounded-xl text-xs font-black border transition-all duration-300 relative ${
                                        ...
                                        : 'bg-white text-gray-400 border-gray-100 hover:border-red-100 transition-all'
                    ...
                    <div className="max-w-6xl w-full grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
                        <div className="space-y-10">
                            <div className="space-y-4">
                                ...
                                <h1 className="text-5xl font-black text-gray-900 leading-tight tracking-tight">Thực hiện <br /><span className="text-[#5B0019]">Xác thực AI</span></h1>
                                <p className="text-gray-500 text-lg leading-relaxed font-medium">Vui lòng căn chỉnh khuôn mặt vào giữa khung hình. Hệ thống AI sẽ theo dõi và ghi lại các hành vi vi phạm trong suốt quá trình làm bài.</p>
                            </div>
                            ...
                            <button
                                ...
                                className={`w-full py-6 rounded-3xl font-black text-xl shadow-2xl transition-all transform hover:scale-[1.02] active:scale-[0.98] flex items-center justify-center gap-4 ${
                                    cameraStatus === "Ready" ? "bg-[#5B0019] text-white hover:bg-black" : "bg-gray-100 text-gray-400 cursor-not-allowed border border-gray-200"
                                }`}
                            >
                                {cameraStatus === "Ready" ? <>VÀO THI NGAY <ChevronRight size={20} /></> : <>KHỞI TẠO AI... ({cameraStatus})</>}
                            </button>
                        </div>
                        <div className="relative flex items-center justify-center">
                            <div className="relative group">
                                ...
                                <div 
                                    ref={centerTargetRef} 
                                    className={`w-[480px] aspect-video rounded-[3rem] overflow-hidden border-8 transition-all duration-700 shadow-2xl relative ${
            ...
            {examStep === 'key' && !isSubmitted && (
                <div className="fixed inset-0 z-[10001] bg-white flex items-center justify-center p-6 animate-in fade-in slide-in-from-right-8 duration-500">
                    <div className="max-w-lg w-full space-y-10">
                        <div className="space-y-4">
                            <button onClick={() => setExamStep('camera')} className="flex items-center gap-2 text-gray-400 hover:text-gray-700 font-bold text-sm transition-colors"><ArrowLeft size={18} /> Quay lại kiểm tra camera</button>
                            <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-amber-50 text-amber-700 rounded-full text-[10px] font-black uppercase tracking-widest border border-amber-100"><KeyRound size={14} /> Xác thực mã đề thi</div>
                            <h1 className="text-5xl font-black text-gray-900 leading-tight tracking-tight">Nhập <br /><span className="text-[#5B0019]">Mã Đề Thi</span></h1>
                            <p className="text-gray-500 text-lg leading-relaxed font-medium">Vui lòng nhập mã đề thi được giáo viên cung cấp để tiếp tục vào bài thi.</p>
                        </div>
                        <div className="space-y-4">
                            <div className={`relative rounded-3xl border-2 transition-all duration-300 overflow-hidden ${
                                keyError ? 'border-red-400 bg-red-50/30' : keyInput.length > 0 ? 'border-[#5B0019]/40 bg-[#5B0019]/3' : 'border-gray-200 bg-gray-50'
                            }`}>
                                <input
                                    type="text"
                                    maxLength={12}
                                    ...
                                    className="w-full px-8 py-7 bg-transparent border-none text-4xl font-black tracking-[0.4em] text-center text-[#5B0019] placeholder:text-gray-300 focus:outline-none"
                                />
                                {keyInput.length > 0 && <button onClick={() => setKeyInput('')} className="absolute right-5 top-1/2 -translate-y-1/2 p-2 text-gray-300 hover:text-gray-500"><X size={20} /></button>}
                            </div>
                            {keyError && <div className="flex items-center gap-3 p-4 bg-red-50 border border-red-100 rounded-2xl animate-in slide-in-from-top-2 duration-300"><AlertTriangle size={18} className="text-red-500 shrink-0" /><p className="text-sm font-bold text-red-600">{keyError}</p></div>}
                        </div>
                        <button
                            ...
                            className={`w-full py-6 rounded-3xl font-black text-xl shadow-2xl transition-all duration-500 flex items-center justify-center gap-4 ${
                                keyInput.length >= 1 && !keyLoading ? 'bg-[#5B0019] text-white hover:bg-black scale-[1.02] active:scale-[0.98]' : 'bg-gray-100 text-gray-400 cursor-not-allowed border border-gray-200'
                            }`}
                        >
                            {keyLoading ? <><Loader2 className="animate-spin" size={22} /> Đang xác thực...</> : <>XÁC NHẬN & VÀO THI <KeyRound size={20} /></>}
                        </button>
                    </div>
                </div>
            )}
            ...
            {showCheatModal && (
                <div className="fixed inset-0 z-[11000] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-500">
                    <div className="bg-white rounded-[3rem] shadow-2xl max-w-md w-full overflow-hidden border border-red-100 relative translate-y-[-20%] animate-in slide-in-from-bottom-10">
                        <div className="p-10 text-center space-y-8">
                            <div className="w-24 h-24 bg-red-50 text-red-500 rounded-full flex items-center justify-center mx-auto shadow-inner animate-pulse"><AlertTriangle size={48} /></div>
                            <div className="space-y-3">
                                <h2 className="text-3xl font-black text-gray-900 tracking-tight">PHÁT HIỆN GIAN LẬN</h2>
                                <p className="text-gray-500 font-bold leading-relaxed px-4">AI đã phát hiện hành vi bất thường và bài thi của bạn đã bị khóa.</p>
                            </div>
                            <div className="bg-red-500 text-white py-4 px-10 rounded-2xl font-black text-lg inline-block shadow-xl">THOÁT RA SAU {exitCountdown}S</div>
                        </div>
                    </div>
                </div>
            )}
            ...
            {isSubmitted && !showCheatModal && (
                <div className="fixed inset-0 z-[11000] bg-white flex flex-col items-center justify-center p-6 text-center animate-in zoom-in duration-500">
                    <div className="w-24 h-24 bg-green-100 text-green-600 rounded-full flex items-center justify-center mb-6 shadow-xl animate-bounce"><Trophy size={48} /></div>
                    <h1 className="text-4xl font-black text-gray-900 tracking-tight mb-2">Bài thi đã hoàn thành!</h1>
                    <p className="text-gray-500 font-medium mb-12 italic">Hệ hệ thống đang xử lý kết quả của bạn.</p>
                    <div className="bg-[#5B0019] text-white py-4 px-16 rounded-2xl font-black text-lg shadow-2xl">THOÁT SAU {exitCountdown}S</div>
                </div>
            )}
            ...
            <div className="hidden" aria-hidden="true">
                {user?.id && (
                    <div ref={cameraSourceRef} className="w-full h-full">
                        <CameraMonitor onViolation={handleViolation} onStatusChange={setCameraStatus} isCheck={examStep !== 'exam'} isActive={!isSubmitted && !showCheatModal} examId={examId} studentId={user.id} />
                        <ProctoringLockdown isActive={isLockdownActive && examStep === 'exam'} onFullscreenExit={() => { setShowFullscreenWarning(true); setTimeout(() => setShowFullscreenWarning(false), 3000); }} contentSelector="main" />
                    </div>
                )}
            </div>
        </div>
