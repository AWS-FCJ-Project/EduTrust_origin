...
import { X, Upload, Loader2, User, Mail, Shield, BookOpen, GraduationCap } from 'lucide-react';
...
export default function UserProfileModal({ user, isOpen, onClose, onUpdate }: UserProfileModalProps) {
    const defaultStudyImg = '/study.png';  // fallback
        ...
        reader.onloadend = async () => {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    setAvatarPreview(user?.avatar_url || null); // Revert
                }
            } catch (error) {
                        ...
                        <div className="relative mb-4">
                            <div className="w-32 h-32 rounded-full border-4 border-white shadow-lg overflow-hidden bg-slate-100 relative">
                                {avatarPreview ? (
                                    ...
                                    <img src={defaultStudyImg} alt="Avatar" className="w-full h-full object-cover" />
                                )}
                                {isUploading && (
